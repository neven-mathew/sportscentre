...
def book():
    try:
        ...
        # Validate inputs
        if not all([name, email, sport, turf, slot, booking_date]):
        ...
        # Ensure status and email columns exist
        cursor.execute("SHOW COLUMNS FROM bookings LIKE 'status'")
        if not cursor.fetchone():
            cursor.execute("ALTER TABLE bookings ADD COLUMN status VARCHAR(20) DEFAULT NULL")
            db.commit()
        ...
        cursor.execute("""
            INSERT INTO bookings (name, sport, turf, slot_time, booking_date, status, email)
            VALUES (%s, %s, %s, %s, %s, 'pending', %s)
        """, (name, sport, turf, slot, booking_date, email))
        ...
        try:
            ...
            msg.body = f"Hello {name},\n\nYour booking request for {sport} at {turf} on {booking_date} for the {slot} slot has been received and sent to the admin.\n\nPlease wait for confirmation. We will email you once the admin approves your request.\n\nThank you!"
            mail.send(msg)
        except Exception as e:
...
def confirm_booking(id):
    try:
        ...
        # Pull specific fields to ensure we capture the email
        cursor.execute("SELECT name, sport, turf, slot_time, booking_date, email FROM bookings WHERE id=%s AND status='pending'", (id,))
        ...
        name, sport, turf, slot_time, booking_date, email = b_data
        
        cursor.execute("UPDATE bookings SET status='confirmed' WHERE id=%s", (id,))
        db.commit()
        
        # --- SEND CONFIRMATION EMAIL WITH PAYMENT INFO ---
        if email:
            try:
                msg = Message('Booking Confirmed - Payment Required', recipients=[email])
                msg.body = f"Hello {name},\n\nGood news! Your booking for {sport} at {turf} on {booking_date} for the {slot_time} slot has been CONFIRMED.\n\nTo finalize your booking, please send an advance payment of Rs. {ADVANCE_AMOUNT} to the following number via UPI/GPay:\n\nPayment Number: {PAYMENT_NUMBER}\n\nPlease keep the screenshot of your payment for verification at the turf.\n\nThank you!"
                mail.send(msg)
            except Exception as e:
                print(f"Failed to send confirmation email: {e}")

        flash(f'✅ Booking confirmed for {name} on {booking_date} at {slot_time}', 'success')
...
def reject_booking(id):
    try:
        ...
        # Pull specific fields to ensure we capture the email before deleting
        cursor.execute("SELECT name, sport, turf, slot_time, booking_date, email FROM bookings WHERE id=%s AND status='pending'", (id,))
        b_data = cursor.fetchone()
        ...
        if email:
            try:
                msg = Message('Booking Request Update - Sports Center', recipients=[email])
                msg.body = f"Hello {name},\n\nWe regret to inform you that your booking request for {sport} at {turf} on {booking_date} for the {slot_time} slot could not be confirmed at this time (the slot may be unavailable or under maintenance).\n\nPlease try booking a different slot on our website.\n\nThank you!"
                mail.send(msg)
            except Exception as e:
                print(f"Failed to send rejection email: {e}")
        
        flash(f'❌ Booking rejected for {name}', 'info')
